	...
	public static function CRITBINOM($trials, $probability, $alpha) {
		...
		if ((is_numeric($trials)) && (is_numeric($probability)) && (is_numeric($alpha))) {
			...
			if ($alpha <= 0.5) {
				...
				$trialsApprox = 0 - ($t + (2.515517 + 0.802853 * $t + 0.000328 * $t * $t) / (1 + 1.432788 * $t + 0.189269 * $t * $t + 0.001308 * $t * $t * $t));
			} else {
				$t = sqrt(log(1 / pow(1 - $alpha,2)));
				$trialsApprox = $t - (2.515517 + 0.802853 * $t + 0.000328 * $t * $t) / (1 + 1.432788 * $t + 0.189269 * $t * $t + 0.001308 * $t * $t * $t);
			}

...
function AddBidButler($aucid, $newtime, $aucusefree) {
    ...
    $rettime = $newtime;
    $bids_to_take = db_fetch_object(db_query("select bids_to_take from auction where auctionID = $aucid"));
    ...
    if ($newtime >= 0) {
	    ...
	    $plusprice = $plusprice * $bids_to_take;
	    ...
	    while($m <= $bids_to_take){
                ...
                if (!db_query("update bidbutler set used_bids=used_bids+$bids_to_take where id=$id")) {
                    rollback();
                    echo "test16Failed";
                    return -1;
                }

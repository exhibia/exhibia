<?php if(!defined('INST_BASEDIR')) die('Direct access is not allowed!');
    ...
    *  ================================================================= */
	...
	define('STEP_TEMPLATE',		'templates');
	...
	define('STEP_CUSTOM','special');
	define('STEP_CREATE_CRON',	'creatingusercronjob');
	define('STEP_WRITECONFIG',	'writeconfig');
	define('ADD_EMAIL',	'addemail');
	...
	$steps = array(
	/*--------------------------------------------------------------------|
	|  Is user hosted by PAS? Then lets be sure their email works                                                  |
	|---------------------------------------------------------------------|*/
	ADD_EMAIL => array(
			  'title' => 'Adding Email Server and Domain',
			  'enabled' => true,
			  'autoskip' => false
		    ),
	...
	STEP_MODULECHECK => array(
		'title'    =>  'Penny Auction Soft 7.0 Installer',
		'enabled'  =>  true,
		'autoskip' =>  true,
		'modules'  =>  array('mysql','gd', 'curl', 'ftp', 'json', 'xml', 'sockets', 'bcmath', 'mbstring', 'xsl', 'mysql'),
		),
	...
	STEP_CUSTOM=> array(
		'title'        =>  'Site Variables Configuration',
		'enabled'      =>  true,
                'autoskip'     =>  false,
		),
	...
	STEP_CREATE_CRON => array(
	
		'title' => 'Create Cron Job (System Jobs)',
		'enabled' => true,
		'autoskip' => false,
		),
...
//
	...
	\--------------------------------------------------------------------*/
	'installer_title_name' => 'Penny Auction Soft Installer',
	...
	|---------------------------------------------------------------------|
	|  The function "db_query()" does not support multi-query execution
	...
	'supported'    =>  array(							
        ...
        'company' => 'Penny Auction Soft Inc',
        'product' => 'PennyAuctionSoft',
        'version' => '7.1',
		...
		'datenow'  => date('H:m:s, F j, Y'),
		  ...
		  'sitename'=>ucfirst(str_replace("www.", "", preg_replace("/\\.[^.\\s]{2,4}$/", "", $_SERVER['SERVER_NAME']))),
		  'sitenm'=>ucfirst(str_replace("www.", "", preg_replace("/\\.[^.\\s]{2,4}$/", "", $_SERVER['SERVER_NAME']))),
		  'siteurl'=> $_SERVER ['SERVER_NAME'],
		  'pagetitle'=>'Welcome To ' . $_SERVER['SERVER_NAME'],
		  'metatagskeywords'=>ucfirst(str_replace("www.", "", preg_replace("/\\.[^.\\s]{2,4}$/", "", $_SERVER['SERVER_NAME']))) . ', penny, auction, website, great, deals, up to 99% off, fun',
		  'metatagsdescription'=>'Up to 99% Off on amazing products at ' . ucfirst(str_replace("www.", "", preg_replace("/\\.[^.\\s]{2,4}$/", "", $_SERVER['SERVER_NAME']))),
		  'currencysymbol'=>'$',
		  'currencycode'=>'USD',
	...
	'template' => $steps['templates']['template']

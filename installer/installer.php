...
function change_sql($new_string, $string_to_replace){
$installer = 'set';
include("config/config.inc.php");
  ...
  $tables_result = db_query($sql);

  if (!$tables_result) {
   // echo "Database error, could not list tables\nMySQL error: " . db_error();
    //exit;
  }

  //echo "In these fields '$string_to_replace' have been replaced with '$new_string'\n\n";
  while ($table = db_fetch_row($tables_result)) {
    //echo "Table: {$table[0]}\n";
    $fields_result = db_query("SHOW COLUMNS FROM ".$table[0]);
    if (!$fields_result) {
     // echo 'Could not run query: ' . db_error();
      //exit;
    }
    if (db_num_rows($fields_result) > 0) {
      while ($field = db_fetch_assoc($fields_result)) {
    ...
    else
    {
        ...
        {   			
			...
			if($steps[STEP_IOFILES]['enabled'])
			{
				...
				$user_perms = '0';
				foreach($user_folder as $value){
				
				      if( ! is_writable(INST_BASEDIR . '' . $value)){
					  $page->ErrorBox(INST_BASEDIR . '' . $value . " Is not writeable please correct this <br /> (ie..chmod 775, chown ". $user['name'] . "/ -R<br />" . INST_BASEDIR . '' . $value . "/ -R<br /> Before continuing");
					  $user_perms = 0;
					  $ioable = false;
					  $status['state'] = 'failed';
			...
			*/
			...
			// If the template step is enabled and active, show "select template" form
			
			if($steps[STEP_TEMPLATE]['enabled'] && $step == STEP_TEMPLATE)
			{
				$page->MainTitle($steps[STEP_TEMPLATE]['title'], 'template');
				$page->Paragraph('Select one of the following supported templates:');

				// Begin the TEMPLATE selection
				$page->FormStart(array('step'=>GetNextStep(STEP_TEMPLATE)));
				if(preg_match("/public_html/", $_SERVER['DOCUMENT_ROOT'])){
				$folder = str_replace("/", "", str_replace("public_html", "", str_replace("home", "", $_SERVER['DOCUMENT_ROOT'])));
				    $user = array();
				    $user['name'] = $folder;
				}else{
				...
				$page->Label('Apache and System User');
				$page->FormInput($user['name'], 'username');
				//$page->FormInput($login['username'], 'username');
				 $page->Label('Template');
				 $page->FormSelect($steps[STEP_TEMPLATE]['templates'],$steps[STEP_TEMPLATE]['template'], 'template');

				$prev = GetPrevStep(STEP_TEMPLATE);
				if($prev) $page->FormButton('Back', array('step'=>$prev));   
                                     
                $page->FormSubmit('Next');
                $page->FormClose();
				$page->ShowPage(STEP_TEMPLATE);
			}
                        /* ===================================================[ Custom ]=================================================== */
...
//				$validPrefix = $dbase->IsDatabaseFriendly($prefix);
			...
			if($steps[STEP_CUSTOM]['enabled'] && ($step == STEP_CUSTOM))
			{
                            // Get the prefix value to variable
                            $sitename = $keywords['special']['sitename'];
                            $siteurl = $_SERVER['SERVER_NAME'];
                            $pagetitle=$keywords['special']['pagetitle'];
                            $metatagskeywords=$keywords['special']['metatagskeywords'];
                            $metatagsdescription=$keywords['special']['metatagsdescription'];
                            $currencysymbol=$keywords['special']['currencysymbol'];
                            $currencycode=$keywords['special']['currencycode'];
			    $sitename=$keywords['special']['sitename'];
			    $sitenm=$keywords['special']['sitenm'];
			    
			// The prefix checks are too much for one if statement,
			// so the truth value is stored in $validPrefix                          

				$page->MainTitle($steps[STEP_CUSTOM]['title'], 'custom');
				...
				// the prefix is not optional and MUST have a value
                                ...
                                $currencylist=array('USD','EUR','AUD','CAD');
                                

                                require('assets'.DIRECTORY_SEPARATOR.'helper.custom.php');
				if($step != STEP_CUSTOM)
				{
					//if(strlen($siteurl) == 0)
                                           // $page->WarningBox('Please enter the invalid url of your site');

                                        if(strlen($sitename)==0)
                                            $page->WarningBox('The sitename is empty, Please enter the name of your site');

                                        if(strlen($pagetitle)==0)
                                            $page->WarningBox('The title of each page is empty, Please enter the title');

                                        if(strlen($metatagskeywords)==0)
                                            $page->WarningBox('The metatag keyword is empty, Please enter the keywork');

                                        if(strlen($metatagsdescription)==0)
                                            $page->WarningBox('The metatag description is empty, Please enter the description');
				}
				...
				else
				{
                                    ...
                                    if(isset($_REQUEST['create']) && $_REQUEST['create'] == 'custom')
                                    {
                                        $valid=true;
                                        ...
                                        if(strlen($sitename)==0)
                                        {
                                            $page->ErrorBox('The sitename is empty, Please enter the name of your site');
                                            $valid=false;
                                        }

                                        if(strlen($pagetitle)==0)
                                        {
                                            $page->ErrorBox('The title of each page is empty, Please enter the title');
                                            $valid=false;
                                        }

                                        if(strlen($metatagskeywords)==0)
                                        {
                                            $page->ErrorBox('The metatag keyword is empty, Please enter the keywork');
                                            $valid=false;
                                        }

                                        if(strlen($metatagsdescription)==0)
                                        {
                                            $page->ErrorBox('The metatag description is empty, Please enter the description');
                                            $valid=false;
                                        }
					...
					if($valid == true)
					{
						$page->SuccessBox('All the Site Configuration is accepted. Proceed to the next step.');

                                                $page->FormStart(array('step'=>GetNextStep(STEP_CUSTOM)));

                                                // Buttons to create the admin and go back
                                                $page->FormButton('Back', array('step'=>GetPrevStep(STEP_CUSTOM)));
                                                $page->FormSubmit('Next');
                                                $page->FormClose();
                                                $page->ShowPage(STEP_CUSTOM);
					}
                                        else
                                        {
                                            $page->FormStart(array('step'=>STEP_CUSTOM, 'create'=>'custom'));
                                            //$page->FormStart(array('step'=>GetNextStep(STEP_CUSTOM)));
                                            $page->Label('Site Name:');
                                            $page->FormInput($sitename, 'sitename'); ## <<<<< MUST MATCH THE SPECIAL KEYWORD INDEX TO WORK!!!!

                                          //  $page->Label('Site Url:');
                                         //   $page->FormInput($siteurl, 'siteurl');
					    $page->Label('Site Short Name:');
                                            $page->FormInput($sitenm, 'sitenm');
                                            $page->Label('Page Title:');
                                            $page->FormInput($pagetitle, 'pagetitle');

                                            $page->Label('Currency Symbol');
                                            $page->FormInput($currencysymbol, 'currencysymbol');

                                            $page->Label('Currency Code');
                                            $page->FormSelect($currencylist,$currencycode, 'currencycode');

                                            
                                            $page->Label('Language');
                                        foreach($keywords['special']['supported'] as $langCode => $langName){
					    $flag = $page->GetFlagHtml($langCode);
					     if($langCode == $keywords['special']['language']){
						
					    $page->FormRadiobox($langCode, 'language', '&nbsp;'.$flag.'&nbsp;'.$langName, array('checked'));
					    }else{
					    
					    $page->FormRadiobox($langCode, 'language', '&nbsp;'.$flag.'&nbsp;'.$langName, array(''));
					    }
                                           }
                                           
                                           $page->Label('Metatags Keywords:');
                                            $page->FormInput($metatagskeywords, 'metatagskeywords');

                                            $page->Label('Metatags Description:');
                                            $page->FormInput($metatagsdescription, 'metatagsdescription');

                                            $page->FormButton('Back', array('step'=>GetPrevStep(STEP_CUSTOM)));
                                            $page->FormSubmit('Submit');
                                            $page->FormClose();
                                            $page->ShowPage(STEP_CUSTOM);
                                        }
                                    ...
                                    else
                                    {
                                        ...
                                        $page->Paragraph('Please enter the right parameter for your site.');
                                        //isset($_REQUEST['create']) && $_REQUEST['create'] == 'administrator'
                                        $page->FormStart(array('step'=>STEP_CUSTOM, 'create'=>'custom'));
			...
			if($steps[STEP_CREATE_CRON]['enabled'])
			{
				...
				$handle = file_get_contents(INST_BASEDIR . 'cron.sh');
				
				$handle = preg_replace("#/(.*)/public_html/#", INST_BASEDIR, $handle);
				$handle = preg_replace("#/(.*)/html/#", INST_BASEDIR, $handle);
				$handle = preg_replace("#/(.*)/htdocs/#", INST_BASEDIR, $handle);
				ini_set('display_errors', 1);
				
				$handle = str_replace("dev_server", $user['name'], $handle);
				
				shell_exec('chmod 777 ' . INST_BASEDIR . 'cron.sh');
				file_put_contents(INST_BASEDIR . 'cron.sh' , $handle);
				
		      
				
			
			  
			//  if(!preg_match("/cron.sh/", $output)){
				
			//	shell_exec("crontab -l | { cat; echo '*/5    *    *    *    *    " .INST_BASEDIR . "cron.sh" . "'; } |crontab -");
			if($user['name'] != 'apache'){
			    shell_exec("rm /var/spool/cron/" . $user['name']);
			    shell_exec("touch /var/spool/cron/" . $user['name']);
			    shell_exec("chmod 777 /var/spool/cron/" . $user['name']);
			    shell_exec("echo -e \"*/5    *    *    *    *    " . INST_BASEDIR . "public_html/cron.sh\"  >> /var/spool/cron/" . $user['name']);
			
			
			}else{
			
			
			echo "Problem creating cron script...it will have to be manually done after install. Using the following Shell command \n";
			echo "echo -e \"*/5    *    *    *    *    " . INST_BASEDIR . "/cron.sh\"  >> /var/spool/cron/<the username>";
			
			}
			//	}
			//	shell_exec('mv /tmp/cron_test /var/spool/cron/' . $user['name']);
				shell_exec("sh " . INST_BASEDIR . "cron.sh");
				shell_exec("sh ln -s " . INST_BASEDIR . "/uploads  " . INST_BASEDIR . "/img");
				shell_exec("sh ln -s " . INST_BASEDIR . "/uploads  " . INST_BASEDIR . "/image");
				shell_exec("sh ln -s " . INST_BASEDIR . "/uploads  " . INST_BASEDIR . "/images");
			...
			if($steps[STEP_WRITECONFIG]['enabled'])
			{
				...
				$SITE_NAME = preg_replace("/^[a-z]{2,4}\\./", "", preg_replace("/\\.[^.\\s]{2,4}$/", "", $_SERVER['SERVER_NAME']));
				$SITE_NM = preg_replace("/^[a-z]{2,4}\\./", "", preg_replace("/\\.[^.\\s]{2,4}$/", "", $_SERVER['SERVER_NAME']));
				
				$change_sql = array("support@pennyauctionsoft.com" => "support@" . str_replace("www." ,"", $_SERVER['SERVER_NAME']), "support@dev.pennyauctionsoft.com" => "support@" . str_replace("www." ,"", $_SERVER['SERVER_NAME']), "dev.pennyauctionsoft.com" => $_SERVER['SERVER_NAME'], "www.pennyauctionsoft.com" => $_SERVER['SERVER_NAME'], "pennyauctionsoft.com" => $_SERVER['SERVER_NAME'], "dev.pennyauctionsoft" => $keywords['special']['sitename'], "pennyauctionsoft" => $keywords['special']['sitename'], "Penny Auction Soft Dev Server" => $keywords['special']['sitename'], "swoopo clone,swoopo site, telebid action script" => $keywords['special']['metatagskeywords'], "MetaTagsdescripton: Win every day products like ipods, furnature, electronics and gift cards for up to 96% off of retail." => $keywords['special']['metatagsdescription']);
				
				    foreach($change_sql as $key => $value){
				    
					change_sql($value, $key);
			...
			// If writing config is enabled - the code here MUST be executed!
			// but the page might not be needed to be displayed
			if($steps[ADD_EMAIL]['enabled'] == true)
			{
			    
		  
			        db_connect('localhost', 'postfix', 'postfix');
				db_select_db('postfix');
				
				if(db_num_rows(db_query("select * from domain where domain like '%" . str_replace("www.", "", $_SERVER['SERVER_NAME']) . "%'")) == 0){
		    
					db_query("insert into domain values('" . str_replace("www.", "", $_SERVER['SERVER_NAME']) . "','', 10, 10, 10,0, 'virtual', 0, NOW(), NOW(), 1);");
					
					
					db_query("insert into domain values('" . $_SERVER['SERVER_NAME'] . "','', 10, 10, 10,0, 'virtual', 0, NOW(), NOW(), 1);");
				    
				    
					db_query("insert into mailbox values('`support@" . $_SERVER['SERVER_NAME'] . "', 'letmein123', 'Support At $_SERVER[SERVER_NAME]', '" . $_SERVER['SERVER_NAME'] . "/support/', '1024000', 'support', '" . $_SERVER['SERVER_NAME'] . "', NOW(), NOW(), 1);");
					
					
					db_query("insert into mailbox values('support@" . str_replace("www.", "", $_SERVER['SERVER_NAME']) . "', 'letmein123', 'Support At $_SERVER[SERVER_NAME]', '" . str_replace("www.", "", $_SERVER['SERVER_NAME']) . "/support/', '1024000', 'support', '" . str_replace("www.", "", $_SERVER['SERVER_NAME']) . "', NOW(), NOW(), 1);");
			      }

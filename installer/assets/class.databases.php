    ...
    class Inst_Databases
    {
        ...
        function Connect($host='', $user='', $pass='', $port='')
        {
			...
			if(is_array($host))
			{
				...
				$this->connection = db_connect($hostport, $host['username'], $host['password']);            
			}
			else 
			{
				if(strlen($port) > 0)
				...
				$this->connection = db_connect($host, $user, $pass);
			}
        }
        ...
        function RunQuery($query)
        {
			if(is_array($query))
			{
				...
				foreach($query as $q)
				{
					...
					$result = db_query($q, $this->connection);
					if($result)
			...
			{
				return db_query($query, $this->connection);
			}
        }
        ...
        {
            $this->dbselected = db_select_db($database, $this->connection);  
        ...
        function CreateNewDatabase($database)
        {
            ...
            if(db_affected_rows($this->connection) > 0)
                 return true;
            else return false;
        }   

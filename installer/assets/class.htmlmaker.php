<?php if(!defined('INST_BASEDIR')) die('Direct access is not allowed!');
    ...
    *  ================================================================= */
    ...
    class Inst_HtmlMaker
    {
        ...
        function FormSelect($selectlist,$value, $name='input', $optional=array(), $containerAddClass='')
        {
            $attribs = $this->GetAttributeString($optional);
			$containerAddClass = (strlen($containerAddClass) > 0) ? ' '.$containerAddClass : '';
//            $this->queue[] =
//                '<div class="textbox'.$containerAddClass.'"><textarea id="'.$name.'" name="'.$name.'" '.$attribs.' />'.$value.'</textarea></div>'."\n";
            ...
            $tag='<div class="textbox'.$containerAddClass.'">';
            $tag.='<select id="'.$name.'" name="'.$name.'">';
            foreach($selectlist as $item)
            {
                if($item==$value)
                {
                    $tag.='<option selected value="'.$item.'">'.$item.'</option>';
                }
                else
                {
...
?>
		...
		<div id="footer">
			&copy; Penny Auction Soft Inc <?php echo date('Y'); ?> | PennyAuctionSoft Setup Wizard
		</div>
	</div>
</div>

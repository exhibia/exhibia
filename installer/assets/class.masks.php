<?php if(!defined('INST_BASEDIR')) die('Direct access is not allowed!');
    ...
    *  ================================================================= */
    ...
    class Inst_Masks
    {
	    ...
	    if(!is_array($keyword) & !is_array($value)){
		...
		function GetMask($maskname, $replaceKeywords=true)
		{
			...
			if(is_file($file) && is_readable($file))
			{
				...
				$content =str_replace("{siteurl}", "http://' . \$_SERVER['SERVER_NAME'] . '/",  $content);
				//$content =str_replace("{template}", $config[STEP_TEMPLATE]['templates'],  $content);
				if($replaceKeywords)

...
if ($searchdata != "") {
    $qryselCL = "select count(*) as total from auction a left join products p on a.productID=p.productID " .
            "left join categories c on p.categoryID=c.categoryID left join auc_due_table adt on a.auctionID=adt.auction_id " .
            "where a.auc_status='2' and p.name like '%$searchdata%' and a.uniqueauction=1";

    $qryselL = "select adt.auc_due_price,adt.auc_due_time,a.auctionID,a.productID as productID,a.categoryID as categoryID,auc_start_price,auc_final_price,auc_start_date,auc_end_date,auc_start_time,auc_end_time,auc_status,auc_type,auc_recurr,buy_user,
        ...
        (select count(id) from auction_seat st where st.auction_id=a.auctionID) as seatcount " .
            ...
            "where a.auc_status='2' and p.name like '%$searchdata%' and a.uniqueauction=1 order by adt.auc_due_time limit $from, $items_per_page";
} else {

    $qryselCL = "select count(*) as total from auction where auc_status='2' and uniqueauction=1";
    ...
    $qryselL = "select adt.auc_due_price,adt.auc_due_time,a.auctionID,a.productID as productID,a.categoryID as categoryID,auc_start_price,auc_final_price,auc_start_date,auc_end_date,auc_start_time,auc_end_time,auc_status,auc_type,auc_recurr,buy_user,
        ...
        (select count(id) from auction_seat st where st.auction_id=a.auctionID) as seatcount " .
            "from auction a left join products p on a.productID=p.productID and a.bidpack=0 left join bidpack b on b.id=a.productID and a.bidpack=1 left join categories c on p.categoryID=c.categoryID " .
            "left join auc_due_table adt on a.auctionID=adt.auction_id where a.auc_status='2' and a.uniqueauction=1 order by adt.auc_due_time " .
            "limit $from, $items_per_page";
}

$resultCL = db_query($qryselCL) or die("1:====" . db_error());
$totalCL = db_result($resultCL, 0);
db_free_result($resultCL);

$resselL = db_query($qryselL) or die("2:====" . db_error());

$total_pages = ceil($totalCL / $items_per_page); // Figure out the total number of pages. Always round up using ceil()
...
<?php if ($totalCL > 0) { ?>
    ...
    <?php
    $is_header = TRUE;
    while (( $obj = db_fetch_array($resselL))) {
...
<?php } ?>
        ...
        <div class="bid-content">
            <h2>
                <?php if ($obj['uniqueauction'] == false) {
 ?>
                    <a href="<?php echo SEOSupport::getProductUrl($pname, $obj["auctionID"], 'n'); ?>"><?= htmlspecialchars(stripslashes($pname), ENT_QUOTES); ?></a>
<?php } else { ?>
                    <a href="<?php echo SEOSupport::getProductUrl($pname, $obj["auctionID"], 'l'); ?>"><?= htmlspecialchars(stripslashes($pname), ENT_QUOTES); ?></a>
<?php } ?>
            </h2>
            <h3><?php echo DESCRIPTION; ?>:</h3>
            <p><?= htmlspecialchars(choose_short_desc(stripslashes($short_desc), 140), ENT_QUOTES); ?>
<?php if ($obj['uniqueauction'] == false) { ?>
...
<?php } ?>
        ...
        <div class="bid-countdown">
            ...
            <p class="instead"><strong><span id="ubid_index_page_<?= $obj["auctionID"]; ?>">---</span>&nbsp;<?php echo BIDS; ?></strong> (<?php echo VALUE; ?>:<?= $Currency . number_format($price, 2); ?>)</p>
            </div>
        </div><!-- /bid-countdown -->
    </div><!-- /bid-box -->
    <?
            }
            db_free_result($resselL);
    ?>
            <div class="clear" style="height: 15px;">&nbsp;</div>
            <div class="imagetital" align="center">
        <?
            $get_param = $searchdata != "" ? "st=" . htmlspecialchars(stripcslashes($searchdata), ENT_QUOTES) : ($aid == 4 ? "aid=5" : "id=$id");

            if ($PageNo > 1) {
                $npage = $PageNo - 1;
        ?>
                <span><a class="darkblue-12-link" href="allauctions.php?pgno=1&<?= $get_param; ?>" style="text-decoration: none;">&lt;&lt;</a></span>

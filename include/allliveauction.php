...
if ($searchdata != "") {
    $qryselC1 = "select count(*) as total from auction a left join products p on a.productID=p.productID " .
            ...
            "where a.auc_status='2' and p.name like '%$searchdata%' and a.uniqueauction=0";
...
$qryselC1 = "select count(*) as total from auction where auc_status='2' and categoryID=$id and uniqueauction=0";
        ...
        minseats,
        maxseats,
        seatbids,lockauction,bidpack,tax1,tax2,bidpack_name,bidpack_banner,bidpack_price,price,bid_size,freebids ,p.name,p.picture1,short_desc where a.auctionID NOT IN (" . $_GET['auctionlist'] . ")";
                
                 $qrysel1 .= " order by adt.auc_due_time limit 1";
echo $qrysel1;
}else{
    if ($aid > 0) {
        $qryselC1 = "select count(*) as total from auction a left join auc_due_table adt on a.auctionID=adt.auction_id " .
                "where a.auc_status='$aid' and adt.auc_due_time and a.uniqueauction=0";
        ...
        (select count(id) from auction_seat st where st.auction_id=a.auctionID) as seatcount " .
                ...
                "left join categories c on p.categoryID=c.categoryID left join auc_due_table adt on a.auctionID=adt.auction_id " .
                "where a.auc_status='$aid' and adt.auc_due_time and a.uniqueauction=0 order by adt.auc_due_time limit $from, $items_per_page";
    } else {
        $qryselC1 = "select count(*) as total from auction where auc_status='2' and categoryID=$id and uniqueauction=0";

        $qrysel1 = "select adt.auc_due_price,adt.auc_due_time,a.auctionID,a.productID as productID,a.categoryID as categoryID,auc_start_price,auc_final_price,auc_start_date,auc_end_date,auc_start_time,auc_end_time,auc_status,auc_type,auc_recurr,buy_user,
        auc_fixed_price,fixedpriceauction,pennyauction,nailbiterauction,offauction,cashauction,reserve,nightauction,openauction,time_duration,auc_final_end_date,total_time,pause_status,shipping_id,future_tstamp,recurr_count,auction_min_price,min_win_price,c.name as catname,
...
ob_start();
    ...
    $is_header = TRUE;
    while (( $obj = db_fetch_array($ressel1))) {
    ...
    ?>
            ...
            <div class="auction_subtitle">
                <strong>
            <?
            if ($obj["catname"] == "" || $searchdata != "") {
                echo ALL_AUCTIONS;
            } else {
                echo htmlspecialchars(stripcslashes($obj["catname"]), ENT_QUOTES);
            }
            ?>
        </strong>
    </div>
    ...
    ?>
        ...
        ?>
                ...
                <span class="blue_link"><a class="darkblue-12-link" href="allauctions.php?pgno=<?= $npage; ?>&<?= $get_param; ?>" style="text-decoration: none;">&lt;</a></span>
        <?
            } else {
        ?>
                <span>&lt;&lt;</span><span>&lt;</span>
        <?php } ?>
            ...
            if ($PageNo < $total_pages) {
                $npage = $PageNo + 1;
            ?>
                <span> |<a class="darkblue-12-link" href="allauctions.php?pgno=<?= $npage; ?>&<?= $get_param; ?>" style="text-decoration: none;">&gt;</a></span>
...
//send buffer
echo "{'aid' : '" . $obj['auctionID'] ."', 'text' : '" . addslashes(ob_get_contents()) . "'}";
ob_end_flush();

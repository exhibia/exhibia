...
if ($searchdata != "") {
    ...
    if ($aid > 0) {
        $qryselC1 = "select count(*) as total from auction a left join auc_due_table adt on a.auctionID=adt.auction_id " .
                "where a.auc_status='$_REQUEST[aid]' and adt.auc_due_time!=0 and a.uniqueauction=0";
        ...
        (select count(id) from auction_seat st where st.auction_id=a.auctionID) as seatcount " .
                ...
                "left join categories c on p.categoryID=c.categoryID left join auc_due_table adt on a.auctionID=adt.auction_id " .
                "where a.auc_status='$_REQUEST[aid]' and adt.auc_due_time!=0 and a.uniqueauction=0 order by adt.auc_due_time limit $from, $items_per_page";
    } else {
...
$picture = $objauc['bidpack_banner'];
                                $objauc['name'] = $objauc['bidpack_name'];
                               
                                $objauc['price'] = $objauc['bidpack_price'];
...
if(!empty($objauc['bidpack_name'])){
	...
	$objauc['picture1'] = $objauc['bidpack_banner'];
        ...
        $objauc['price'] = $objauc['bidpack_price'];
        $objauc['short_desc'] = "{$objauc['bid_size']} Bids and {$objauc['freebids']} Freebids";

}


if(empty($objauc['picture1'])){

$objauc['picture1'] = 'no_image.jpg';


}
    $objauc['picture1'] = urlencode($objauc['picture1']);        
    ?>
 ...
 <?php if ($cornerImag != '') {
				      ...
				      <div class="corner_imagev">
                                        <img src="include/addons/icons/quibids-1.0/<?php echo $cornerImag; ?>"  alt="" style="margin-bottom: -73px;margin-top: -1px;"/>
                                    </div>

                            <?php } ?>

                            <a href="viewproduct.php?aid=<?php echo $objauc["auctionID"]; ?>" class="thumb" style="background-image: url('<?php echo $UploadImagePath; ?>/products/thumbs/thumb_<?php echo $objauc["picture1"]; ?>');"></a>
                            ...
                            <div class="price-bidder">
                                <span><?php echo $Currency ;?><label id="price_index_page_<?php echo $objauc["auctionID"]; ?>">---</label> <?php echo $CurrencyName;?></span>
                                <span><?php echo $Currency ;?><a id="value_index_page_<?php echo $objauc["auctionID"]; ?>"><?php echo $objauc['price']; ?></a> <?php echo $CurrencyName;?></span>

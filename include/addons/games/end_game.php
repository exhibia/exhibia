...
$last_id = db_fetch_array(db_query("select max(id) from lobby_messages order by id desc limit 1"));
      $last = number_format($last_id[0]) + 1;
      
      db_query("insert into lobby_messages(id, domain, recipient, sender, message, time, alert_type) values(null, '$_REQUEST[domain]', '$_REQUEST[opponent]', '$_REQUEST[domain]', '<span style=\"color:red;\">$_REQUEST[username] has left: $_REQUEST[room]. Click <a href=\"javascript:;\" onclick=\"delete_message(\'" . $last . "\', \'true\');\">Here</a> to leave now.</span>', NOW(), 'alert');");
}else{

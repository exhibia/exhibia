...
var files_xml = {
<?php
$sql = db_query("SELECT * FROM trivia_questions ORDER BY question asc");
$first = 0;
while($row = db_fetch_array($sql)){
      if($first == 1){
	$str .= ", ";
      }
      $str .= "'" . $row['question'] . "':" . $row['id'] ."'";
$first = 1;
}
?>
};
...
var files_json = {
  //'General':'general_json.txt', 'Animals':'animals_json.txt'
<?php
$sql = db_query("SELECT * FROM trivia_questions ORDER BY question asc");
$first = 0;
while($row = db_fetch_array($sql)){
	...
	$str .= ", ";
      ...
      $str .= "'" . $row['question'] . "':answers.php?id" . $row['id'] ."'";
$first = 1;
}
?>

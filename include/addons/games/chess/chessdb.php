...
*/
	...
	function loadHistory()
	{
		...
		$allMoves = db_query("SELECT * FROM " . $CFG_TABLE[history] . " WHERE gameID = ".$_SESSION['gameID']." ORDER BY timeOfMove");

		$numMoves = -1;
		while ($thisMove = db_fetch_array($allMoves, MYSQL_ASSOC))
		{
			$numMoves++;
	...
	function savePromotion()
	{
		...
		if ($CFG_USEEMAILNOTIFICATION && ! $_SESSION['isSharedPC'])
		{
			...
			if ($oppColor == 'white')
				$tmpOpponentID = db_query("SELECT whitePlayer FROM " . $CFG_TABLE[games] . " WHERE gameID = ".$_SESSION['gameID']);
			else
				$tmpOpponentID = db_query("SELECT blackPlayer FROM " . $CFG_TABLE[games] . " WHERE gameID = ".$_SESSION['gameID']);
			...
			if (db_num_rows($tmpOpponentEmail) > 0)
			{
				...
				{
					/* get opponent's nick */
					$tmpOpponentNick = db_query("SELECT nick FROM " . $CFG_TABLE[players] . " WHERE playerID = ".$_SESSION['playerID']);
					$opponentNick = db_result($tmpOpponentNick, 0);
					
					/* get opponent's prefered history type */
					$tmpOpponentHistory = db_query("SELECT value FROM " . $CFG_TABLE[preferences] . " WHERE playerID = ".$opponentID." AND preference = 'history'");
					
					/* default to PGN */
					if (db_num_rows($tmpOpponentHistory) > 0)
						$opponentHistory = db_result($tmpOpponentHistory, 0);
					else
						$opponentHistory = 'pgn';
	...
	function loadGame()
	{
		...
		/* setup board */
		while ($thisPiece = db_fetch_array($pieces, MYSQL_ASSOC))
		{
		...
		$tmpTurns = db_query($tmpQuery);
		$tmpTurn = db_fetch_array($tmpTurns, MYSQL_ASSOC);
		...
		for ($i = 0; $i < 8; $i++)
		{
			...
			for ($j = 0; $j < 8; $j++)
			{
				...
				if ($board[$i][$j] != 0)
				{
					...
					db_query("INSERT INTO " . $CFG_TABLE[pieces] . " (gameID, color, piece, row, col) VALUES (".$_SESSION['gameID'].", '$tmpColor', '$tmpPiece', $i, $j)");
				}
			}
		}
	...
	function processMessages()
	{
		...
		$tmpMessages = db_query($tmpQuery);

		while($tmpMessage = db_fetch_array($tmpMessages, MYSQL_ASSOC))
		{
		...
		if ($_POST['isCheckMate'] == 'true')
			db_query("UPDATE " . $CFG_TABLE[games] . " SET gameMessage = 'checkMate', messageFrom = '".$currentPlayer."' WHERE gameID = ".$_SESSION['gameID']);
		...
		$tmpMessages = db_query($tmpQuery);
		$tmpMessage = db_fetch_array($tmpMessages, MYSQL_ASSOC);

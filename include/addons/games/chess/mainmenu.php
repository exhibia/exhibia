...
*/
	...
	$tmpOldGames = db_query($tmpQuery);

	/* for each older game... */
	while($tmpOldGame = db_fetch_array($tmpOldGames, MYSQL_ASSOC))
	{
		/* ... clear the history... */
	...
	switch($_POST['ToDo'])
	{
		case 'NewUser':
			...
			$existingUsers = db_query($tmpQuery);
			if (db_num_rows($existingUsers) > 0)
			{
				require 'newuser.php';
				die();
			}
		...
		case 'Login':
			...
			$tmpPlayers = db_query($tmpQuery);
			$tmpPlayer = db_fetch_array($tmpPlayers, MYSQL_ASSOC);
			...
			$tmpPreferences = db_query($tmpQuery);
			...
			while($tmpPreference = db_fetch_array($tmpPreferences, MYSQL_ASSOC))
			{
		...
		case 'InvitePlayer':
			...
			if (db_num_rows($tmpExistingRequests) == 0)
			{
				...
				if ($CFG_USEEMAILNOTIFICATION)
				{
					...
					$tmpOpponentEmail = db_query("SELECT value FROM " . $CFG_TABLE[preferences] . " WHERE playerID = ".$_POST['opponent']." AND preference = 'emailNotification'");
					if (db_num_rows($tmpOpponentEmail) > 0)
					{
						$opponentEmail = db_result($tmpOpponentEmail, 0);
						if ($opponentEmail != '')
						{
		...
		case 'WithdrawRequest':
			...
			$tmpOpponentID = db_query("SELECT whitePlayer FROM " . $CFG_TABLE[games] . " WHERE gameID = ".$_POST['gameID']);
			if (db_num_rows($tmpOpponentID) > 0)
			{
				$opponentID = db_result($tmpOpponentID, 0);

				if ($opponentID == $_SESSION['playerID'])
				{
					$tmpOpponentID = db_query("SELECT blackPlayer FROM " . $CFG_TABLE[games] . " WHERE gameID = ".$_POST['gameID']);
					$opponentID = db_result($tmpOpponentID, 0);
				}
				...
				{
					/* if opponent is using email notification... */
					$tmpOpponentEmail = db_query("SELECT value FROM " . $CFG_TABLE[preferences] . " WHERE playerID = ".$opponentID." AND preference = 'emailNotification'");
					if (db_num_rows($tmpOpponentEmail) > 0)
					{
						$opponentEmail = db_result($tmpOpponentEmail, 0);
						if ($opponentEmail != '')
		...
		case 'UpdatePersonalInfo':
			...
			$tmpPassword = db_query($tmpQuery);
			$dbPassword = db_result($tmpPassword, 0);
			...
			<div class="preferences-form">
				...
				<div class="form-block">
						...
						<?php
							...
							$tmpPlayers = db_query($tmpQuery);
							$first = true;
							while($tmpPlayer = db_fetch_array($tmpPlayers, MYSQL_ASSOC))
							{
								echo ('<option ');
								if($first)
								{
				...
				<div class="form-block">
						...
						<tbody id="respInviteTblBdy">
							<?php
								...
								$tmpGames = db_query($tmpQuery);

								$rowNbr = 0;
								if (db_num_rows($tmpGames) == 0)
									echo("<tr><td colspan=\"3\">" . gettext("You are not currently invited to any games") . "</td></tr>\n");
								else
									while($tmpGame = db_fetch_array($tmpGames, MYSQL_ASSOC))
									{
										if($rowNbr %2 == 0)
										...
										/* get white's nick */
										$tmpPlayer = db_query("SELECT nick FROM " . $CFG_TABLE[players] . " WHERE playerID = ".$tmpGame['whitePlayer']);
										$player = db_result($tmpPlayer, 0);
										echo ('</td><td>');
										echo($player);

										/* black's nick */
										$tmpPlayer = db_query("SELECT nick FROM " . $CFG_TABLE[players] . " WHERE playerID = ".$tmpGame['blackPlayer']);
										$player = db_result($tmpPlayer, 0);
										echo ('</td><td>');
										echo($player);
		...
		<div class="preferences">
			<div class="preferences-form">
				...
				<div class="form-block">
					...
					<?php
						$tmpGames = db_query("SELECT * FROM " . $CFG_TABLE[games] . " WHERE gameMessage = '' AND (whitePlayer = ".$_SESSION['playerID']." OR blackPlayer = ".$_SESSION['playerID'].") ORDER BY dateCreated");
						...
						else
						{
							...
							while($tmpGame = db_fetch_array($tmpGames, MYSQL_ASSOC))
							{
								...
								$tmpNumMoves = db_query("SELECT COUNT(gameID) FROM " . $CFG_TABLE[history] . " WHERE gameID = ".$tmpGame['gameID']);
								$numMoves = db_result($tmpNumMoves,0);
...
<?php
	$tmpGames = db_query("SELECT * FROM " . $CFG_TABLE[games] . " WHERE (gameMessage <> '' AND gameMessage <> 'playerInvited' AND gameMessage <> 'inviteDeclined') AND (whitePlayer = ".$_SESSION['playerID']." OR blackPlayer = ".$_SESSION['playerID'].") ORDER BY lastMove DESC");

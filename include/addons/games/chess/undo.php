...
/* these functions deal specifically with undoing a move */
	function doUndo()
	{
		...
		$tmpMaxTime = db_query("SELECT Max(timeOfMove) FROM " . $CFG_TABLE[history] . " WHERE gameID = ".$_SESSION['gameID']);
		$maxTime = db_result($tmpMaxTime,0);
		$moves = db_query("SELECT * FROM " . $CFG_TABLE[history] . " WHERE gameID = ".$_SESSION['gameID']." AND timeOfMove = '$maxTime'");

		/* if there actually is a move... */
		if ($lastMove = db_fetch_array($moves, MYSQL_ASSOC))
		{
			/* if the last move was played by this player */
				...
				$numMoves--;
				db_query("DELETE FROM " . $CFG_TABLE[history] . " WHERE gameID = ".$_SESSION['gameID']." AND timeOfMove = '$maxTime'");

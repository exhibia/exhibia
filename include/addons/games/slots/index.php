...
include("$BASE_DIR/include/games/config.inc.php"); 
}
$games_server = 'http://' . $_SERVER['SERVER_NAME'] . '/';
...
<link rel="stylesheet" href="<?php echo $games_server; ?>include/addons/games/slots/SyntaxHighlighter/Styles/SyntaxHighlighter.css" type="text/css" media="screen" title="no title" charset="utf-8">
<script>
...
.fancy .slot li {
    ...
    border-right: 1px solid #999;
     background: url("<?php echo $games_server; ?>/include/addons/games/slots/image/bg.png") no-repeat;
     position:relative;
     z-index:1;
}
...
background:url("<?php echo $games_server; ?>/include/addons/games/slots/image/lose-bar.png") no-repeat;
position:relative;
top:98px;
z-index:10000;
margin-bottom:-98px;
...
#alert_area{
background: url("<?php echo $games_server; ?>/include/addons/games/slots/image/bg2.png") repeat-x scroll 80px center transparent;
...
#cabinet {
background: url("<?php echo $games_server; ?>/include/addons/games/slots/image/wood.png") repeat scroll 0 0 transparent;
...
color:green;
    ...
    <img src="<?php echo $games_server; ?>/include/addons/games/slots/image/blank-light.gif" style="z-index:1;position:relative;left:300px;width:80px;height:auto;margin-bottom:-2px;" id="top_light" />
    <div id="cabinet">
	    <div id="alert_area"><a><b>Spin To Win!</b></a></div>
	    
            <div class="fancy" style="margin-top:30px;">
            <img src="<?php echo $games_server; ?>/include/addons/games/slots/image/handle.png" id="playFancy" style="cursor:pointer;margin-left:-110px;margin-top:-50px;" />
             ...
             <div id="slots_outer">
                ...
                ?>
                    ...
                    <li alt="<?php echo $key + 1;?>"><span><img src="<?php echo $games_server; ?>/include/addons/games/slots/image/<?php echo $fruit;?>.png" /></span></li>
		   <?php } ?>
                </ul>
             </div>
		...
		<?php
			...
			<?php
			    ...
			    $l = 1;
			     while($l <= $number){ 
			    ?>
				  <img style="width:35px;height:35px;float:left;display:inline;" src="<?php echo $games_server; ?>/include/addons/games/slots/image/<?php echo $fruit;?>.png" />
...
<script src="<?php echo $games_server; ?>/include/addons/games/slots/jquery.easing.1.3.js" type="text/javascript" charset="utf-8"></script>
...
(function($){
	    ...
	    if(finalNumbers){
	     ...
	     if(finalNumbers.length >= 3){
		    ...
		    success: function(response){
			  ...
			  $('#top_light').attr('src', '<?php echo $games_server; ?>include/addons/games/slots/image/flashing-light.gif');
	    ...
	    alert('You need to recharge your account');
		  ...
		  if ( $.isFunction(base.options.onStart) ) {
			...
			base.options.onStart();
		      $('#top_light').attr('src', '<?php echo $games_server; ?>include/addons/games/slots/image/blank-light.gif');
    ...
    $.fn.jSlots = function(options){
     ...
     var audioElementLose = document.createElement('audio');
			audioElementLose.setAttribute('src', '<?php echo $games_server; ?>include/addons/games/slots/sounds/lose.mp3');
			audioElement.setAttribute('src', '<?php echo $games_server; ?>include/addons/games/slots/sounds/lever.mp3');
			audioElementWin.setAttribute('src', '<?php echo $games_server; ?>include/addons/games/slots/sounds/win.mp3');
	...
	function randomFromInterval(from,to)
	  {
	    ...
	    alert_area: '#slot_alert',
            ...
            crossDomain: true,
            ajax_url : '<?php echo $games_server;?>include/addons/games/slots/check_payout.php?userid=<?php echo $_REQUEST['userid'];?>&remote_server=<?php echo urlencode($remote_server); ?>&final=',
	    bids_placed: '#bids_placed',

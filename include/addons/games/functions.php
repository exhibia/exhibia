...
function payout_request($array, $winner, $loser, $BASE_DIR, $gameID, $data){
  ...
  $sql = db_query("select * from in_game where gameID = '$gameID' and last_move = 'won'");
    ...
    $url = "http://" . str_replace("www.", "", $array['domain']) . "/include/addons/games/take_bids.php?username=" . $array['username'] . "&game=" . $array['room'] . "&take_bids=false&gameID=$gameID&reward_with=$connect4[reward_with]&reward=$connect4[reward]&api_token=$api_token[api_token]";

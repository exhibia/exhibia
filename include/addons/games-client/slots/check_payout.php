...
require("$BASE_DIR/include/addons/games-client/config.inc.php");



require("$BASE_DIR/include/addons/games-client/slots/config.inc.php");
require("$BASE_DIR/Functions/update_users_bids.php");
...
$obj = db_fetch_object(db_query("select * from registration where id = $uid"));
	...
	if($master_game_settings['give_bids_back_on_win'] == 0 | ($master_game_settings['give_bids_back_on_win'] == 1 & $flag != 'c')){ 
		      $qryins1 = "Insert into $table_selector" . "account_bidding(user_id, bidpack_buy_date, bid_count,username,position) values('$uid','" . date("Y-m-d H:i:s", time()) . "','$_REQUEST[bids]', '{$obj->username}','{$obj->position}')";
...
echo db_error();
	...
	if($operand == 'add'){	      
		      ...
		      $qryins1 = "Insert into $table_selector" . "account_bidding(user_id, bidpack_buy_date, bid_count, username,position) values('$uid','" . date("Y-m-d H:i:s", time()) . "','$bids', '{$obj->username}','{$obj->position}')";
		      $result = db_query($qryins1);
		    echo db_error();

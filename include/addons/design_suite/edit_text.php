...
<?php
  ...
  <form id="lang_form" name="language_form" action="javascript: submit_test_ajax_form('lang_form'); ">
 ...
 ?>
	...
	<tr>
			  ...
			  Choose Constant(useful for JS that you can't see right away)<br />
			    <select name="choose_constant" id="choose_constant" onchange="change_constant();" style="font-size:12px;word-wrap:break-word;max-width:220px;">
			      ...
			      <?php
				    ...
				    $sqlConstant = db_query("select constant, id from languages where language = 'english' and constant != '' and value != '' and constant not like 'slider%' and constant not like '%define%'");
				    
					while($rowConstant = db_fetch_array($sqlConstant)){
					
					  ?>
					
					    <option style="font-size:12px;word-wrap:break-word;max-width:220px;" value="<?php echo $rowConstant['id'];?>"><?php echo strtolower($rowConstant['constant']);?></option>
	    ...
	    <td colspan="2" valign="top" height="110px">
			      ...
			      <div  contenteditable="true" id="lang_value" id="lang_value" style="height:50px;width:400px;margin-bottom:-50px;background:#fff;border-radius:6px;border:1px inset gray;" ><?php echo stripslashes($rowl['value']);?></div>

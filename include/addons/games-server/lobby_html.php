...
#end_game {
background-image: url("<?php echo $games_server;?>/include/addons/games-server/images/end_game.png");
...
<div id="lobby_outer">
    <div id="chat_output"></div>
      <div id="lobby_box_domain" class="<?php echo $_GET['template']; ?>">
	    ...
	    <ul id="lobby_header">
		<li id="alert"><img src="<?php echo $games_server;?>/include/addons/games-server/alerts.png" /></li>
		<li id="room"><?php echo "<h2>Room: $_GET[room]</h2>"; ?></li>
		
	    </ul>
      <span id="open_lobby"><img src="<?php echo $games_server;?>/include/addons/games-server/arrow-down.png" onclick="open_chat();" /></span>
    </div>
</div>     
 ...
 include("$BASE_DIR/include/addons/games-server/$_REQUEST[room]/index.php");
	  ...
	  if($('#open_lobby img').attr('src')== '<?php echo $games_server;?>/include/addons/games-server/arrow-down.png'){
	      ...
	      $('#open_lobby img').attr('src', '<?php echo $games_server;?>/include/addons/games-server/arrow-down.png');
	...
	function clear_game(username, domain){
	  if($('#stop_game').length == 0){
	    $('#wrapper_game').append('<div id="stop_game" alt="<?php echo $_REQUEST['room'];?>">Ending Game</div>');
	  }
	
	  $.ajax({
		   url: '<?php echo $games_server; ?>/include/addons/games-server/end_game.php',
		   data: { room: $('#end_game').attr('alt'), game: '<?php echo $_REQUEST['game']; ?>', opponent: $('#opponent').val(), gameID : $('#game_instance_id').val(), domain: domain, username: $('#me').val() },
		   type: 'get',
		   crossDomain: true,
	...
	function clear_game_lost(username, domain){
	  if($('#stop_game').length == 0){
	    $('#wrapper_game').append('<div id="stop_game" alt="<?php echo $_REQUEST['room'];?>">Ending Game</div>');
	  }
...
//checks to see if game should begin
	...
	$.ajax({
	      url: '<?php echo $games_server; ?>/include/addons/games-server/<?php echo strtolower(str_replace(" ", "", $_REQUEST['game']));?>/check_game_data.php',
		...
		// $('#end_game').remove();
			...
			if($.isArray(response.opponent)){
			    
			   
			    $('#opponent').remove();
			    $.each(response.opponent, function(i, opponent){
			      if($('#opponent[' + i + ']').length == 0){
				$('#wrapper_game').append('<input id="opponent[' + i + ']" class="opponents" type="hidden" value="' + opponent + '" />');
			      }
		    ...
		    if(response.game != 'game over'){
		      $('#in_game').remove();
		      setTimeout("game_start('" + gameID + "')", 3000);
...
function send_message(me, him, domain, box, msg){
     ...
     if($('#stop_game').length == 0){
	$.ajax({
	      url: '<?php echo $games_server; ?>/include/addons/chat/index.php',
...
function retrieve_chat(me, domain, box){
	 ...
	 setTimeout("retrieve_chat('" + $("#me").val() + "', '<?php echo $_REQUEST['domain']; ?>', '" + box + "')", 7000);
...
function chat_boxes(){
   ...
   $('.opponents').each( function(i, item) {
	...
	$('#lobby_box').prepend('<div class="chat_box" id="chat_box_' + i + '"><a style="color:#fff;" onclick="hide_show(' + i + ');">' + him + '</a><div id="chat_box_div_' + i + '" style="visibility:collapse;" class="chat_holder"><div id="chat_scroll_' + i + '" style="height:230px;"><ul id="messages_holder_' + i + '" class="chat_ul" style="max-height:230px!important;min-height:230px!important;overflow-x:hidden;"></ul></div><textarea alt="' + i + '" name="message_' + i + '" id="message_' + i + '"></textarea><input type="hidden" name="recipient" id="recipient_' + i + '" value="' + him + '" /></div></div>');
      ...
      setTimeout('retrieve_chat(\'' + $("#me").val() + '\', \'<?php echo $_REQUEST['domain']; ?>\', ' + i + ')', 7000);
...
return;
    ...
    if($('#stop_game').length == 0){
    // console.log('<?php echo $games_server; ?>/include/addons/games/get_lobby.php?');
        //   if($('#stop_game').length == 0){
	if(empty(user)){
	user = $('#opponent').val();
	}
	$.ajax({
	  ...
	  url: '<?php echo $games_server; ?>/include/addons/games-server/get_lobby.php?',
	...
	$.ajax({
	  ...
	  url: '<?php echo $games_server; ?>/include/addons/games-server/get_lobby.php?',
	  data: { room : '<?php echo urlencode($_REQUEST['game']); ?>', userid: '<?php echo $_REQUEST['userid'];?>', username: $('#me').val(), remote_server: '<?php echo $games_server; ?>', domain: $('#domain').val(), key : '<?php echo $_REQUEST['key']; ?>', output: 'true', max_players: '<?php echo $_REQUEST['max_players']; ?>', min_players: '<?php echo $_REQUEST['min_players'];?>', accept: user, gameID: gameID },
	  crossDomain: true,
	  method: 'get',
	  dataType: 'jsonp',
	 
	  success: function(response){
           $('#chat_output').fadeOut("slow").css({display:"none"});
	     ...
	     // }
	      ...
	      game_start(response.id);
				  $.ajax({
				      url: '<?php echo $games_server; ?>/include/addons/games-server/get_lobby.php?', // URL to the local file
	...
	$.ajax({
	  ...
	  success: function(response){
		  ...
		  $('#open_lobby img').attr('src', '<?php echo $games_server;?>/include/addons/games-server/arrow-right.png');
	      }
	      parseScript(response);
 ...
 // $('#open_lobby img').css('top', '-15px');
	  ...
	  success: function(response){
		...
		//if($('#open_lobby img').attr('src') != '<?php echo $games_server;?>include/addons/games/arrow-right.png'){
		  $('#open_lobby img').css('top', '-55px');
		  $('#open_lobby img').css('left', '22px');
		  $('#open_lobby img').attr('src', '<?php echo $games_server;?>/include/addons/games-server/arrow-right.png');
		//}

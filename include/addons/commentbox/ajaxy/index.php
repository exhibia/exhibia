...
<!-- PROM BANNERS -->
            ...
            <?php
        $imgpath = dir($BASE_DIR . '/include/addons/commentbox/' . $template . '/img/' . $entry );
        $a = 0;
        while (false !== ($entry = $imgpath->read())) 
		{
            ...
            if (is_file($imgpath->path . '/' . $entry)) {
                ...
                $a++;
                    echo '<div class="commentslider-images"><img src="' . $SITE_URL . '/include/addons/commentbox/' . $template . '/img/' . $entry . '" /></div>';

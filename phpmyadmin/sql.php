...
if (! isset($err_url)) {
    $err_url = (! empty($back) ? $back : $goto)
        ...
        . ((strpos(' ' . $goto, 'mysql_') != 1 && strlen($table))
...
if (isset($_REQUEST['btnDrop']) && $_REQUEST['btnDrop'] == __('No')) {
    ...
    if ($is_gotofile) {
        if (strpos($goto, 'mysql_') === 0 && strlen($table)) {
            $table = '';
        }
        $active_page = $goto;
...
if ((0 == $num_rows && 0 == $unlim_num_rows) || $is_affected) {
    ...
    if ($is_delete) {
        ...
        // and mysql_sql.php has many submit buttons
    ...
    if ($is_gotofile) {
        ...
        include 'libraries/mysql_table_exists.lib.php';

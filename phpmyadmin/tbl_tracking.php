...
$sql_query = " SELECT DISTINCT mysql_name, table_name FROM " .
             PMA_Util::backquote($GLOBALS['cfg']['Server']['pmadb']) . "." .
             PMA_Util::backquote($GLOBALS['cfg']['Server']['tracking']) .
             " WHERE mysql_name = '" . PMA_Util::sqlAddSlashes($GLOBALS['db']) . "' " .
             " ORDER BY mysql_name, table_name";
...
if (PMA_DBI_num_rows($sql_result) > 0) {
    ...
    while ($entries = PMA_DBI_fetch_array($sql_result)) {
        if (PMA_Tracker::isTracked($entries['mysql_name'], $entries['table_name'])) {
            $status = ' (' . __('active') . ')';
        } else {
        ...
        echo '<option value="' . htmlspecialchars($entries['table_name']) . '"' . $s . '>' . htmlspecialchars($entries['mysql_name']) . ' . ' . htmlspecialchars($entries['table_name']) . $status . '</option>' . "\n";
    }
    echo '</select>';
...
$sql_query = " SELECT * FROM " .
     PMA_Util::backquote($GLOBALS['cfg']['Server']['pmadb']) . "." .
     PMA_Util::backquote($GLOBALS['cfg']['Server']['tracking']) .
     " WHERE mysql_name = '" . PMA_Util::sqlAddSlashes($_REQUEST['db']) . "' ".
...
if ($last_version > 0) {
    ...
    while ($version = PMA_DBI_fetch_array($sql_result)) {
        ...
        echo '<tr class="noclick ' . $style . '">';
        echo '<td>' . htmlspecialchars($version['mysql_name']) . '</td>';

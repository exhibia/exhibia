...
$scripts->addFile('mysql_operations.js');

/**
 * Rename/move or copy database
 */
if (strlen($db)
    && (! empty($_REQUEST['mysql_rename']) || ! empty($_REQUEST['mysql_copy']))
) {
    if (! empty($_REQUEST['mysql_rename'])) {
        $move = true;
    } else {
        $move = false;
    }
...
if (empty($is_info)) {
    ...
    $url_query .= '&amp;goto=mysql_operations.php';
...
$_REQUEST['mysql_collation'] = PMA_getDbCollation($db);
...
if ($cfgRelation['pdfwork'] && $num_tables > 0) {
    ...
    $test_query = '
         SELECT *
          ...
          WHERE mysql_name = \'' . PMA_Util::sqlAddSlashes($db) . '\'';

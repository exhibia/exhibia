...
foreach ($fields as $row) {
    ...
    $response->addHTML(
        PMA_getHtmlTableStructureRow(
            ...
            $mysql_is_information_schema, $url_query, $field_encoded, $titles, $table
        )
    );

    if (! $tbl_is_view && ! $mysql_is_information_schema) {
        $response->addHTML(
    ...
    } // end if (! $tbl_is_view && ! $mysql_is_information_schema)
...
$response->addHTML(
    PMA_getHtmlForCheckAllTableColumn(
        ...
        $mysql_is_information_schema, $tbl_storage_engine
    )
);
...
$response->addHTML(
    PMA_getHtmlForOptionalActionLinks(
        $url_query, $tbl_is_view, $mysql_is_information_schema,

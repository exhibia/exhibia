...
if (isset($mode)) {
    ...
    if ('export' == $mode) {
        $sql = "REPLACE INTO " . $pma_table
            . " (mysql_name, table_name, pdf_page_number, x, y)"
            . " SELECT mysql_name, table_name, " . $pdf_page_number_q . ","
    ...
    if ('import' == $mode) {
        PMA_queryAsControlUser(
            ...
            WHERE
            ' . $pmd_table . '.`mysql_name`=' . $pma_table . '.`mysql_name`
            AND
            ' . $pmd_table . '.`table_name` = ' . $pma_table . '.`table_name`
            AND
            ' . $pmd_table . '.`mysql_name`=\''. PMA_Util::sqlAddSlashes($db) . '\'

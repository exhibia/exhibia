...
if ($export_plugin->getProperties()->getForceFile() != null && ! $asfile) {
    ...
    if ($export_type == 'server') {
        ...
        $active_page = 'mysql_export.php';
        include 'mysql_export.php';
    } else {
...
if ($export_type == 'server') {
    ...
    $err_url = 'mysql_export.php?' . PMA_generate_common_url($db);
...
if (! $save_on_server) {
    if ($asfile) {
        ...
        if ($export_type == 'server') {
            ...
            $back_button .= 'mysql_export.php?' . PMA_generate_common_url($db);
        } else {
            $back_button .= 'tbl_export.php?' . PMA_generate_common_url($db, $table);
        }

        // Convert the multiple select elements from an array to a string
        if ($export_type == 'server' && isset($_REQUEST['mysql_select'])) {
            $_REQUEST['mysql_select'] = implode(",", $_REQUEST['mysql_select']);
...
do {
    ...
    if ($export_type == 'server') {
        if (isset($mysql_select)) {
            $tmp_select = implode($mysql_select, '|');

...
$multi_values .= '<a href="#"';
$multi_values .= ' onclick="setSelectOptions(\'dump\', \'mysql_select[]\', true); return false;">';
$multi_values .= __('Select All');
$multi_values .= '</a>';
$multi_values .= ' / ';
$multi_values .= '<a href="#"';
$multi_values .= ' onclick="setSelectOptions(\'dump\', \'mysql_select[]\', false); return false;">';
$multi_values .= __('Unselect All') . '</a><br />';

$multi_values .= '<select name="mysql_select[]" id="mysql_select" size="10" multiple="multiple">';
$multi_values .= "\n";

// Check if the selected databases are defined in $_GET (from clicking Back button on export.php)
if (isset($_GET['mysql_select'])) {
    $_GET['mysql_select'] = urldecode($_GET['mysql_select']);
    $_GET['mysql_select'] = explode(",", $_GET['mysql_select']);
}
...
foreach ($GLOBALS['pma']->databases as $current_db) {
    ...
    if (isset($_GET['mysql_select'])) {
        if (in_array($current_db, $_GET['mysql_select'])) {

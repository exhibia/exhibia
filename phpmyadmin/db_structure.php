...
$scripts->addFile('mysql_structure.js');
...
if (empty($_POST['is_info'])) {
    ...
    if ((!empty($_POST['submit_mult']) && isset($_POST['selected_tbl']))
        ...
        $action = 'mysql_structure.php';
        $err_url = 'mysql_structure.php?'. PMA_generate_common_url($db);

        // see bug #2794840; in this case, code path is:
        // mysql_structure.php -> libraries/mult_submits.inc.php -> sql.php
        // -> mysql_structure.php and if we got an error on the multi submit,
    ...
    include 'libraries/mysql_common.inc.php';
    $url_query .= '&amp;goto=mysql_structure.php';

    // Gets the database structure
    $sub_part = '_structure';
    include 'libraries/mysql_info.inc.php';
...
$mysql_collation = PMA_getDbCollation($db);
...
if ($num_tables == 0) {
    ...
    if (empty($mysql_is_information_schema)) {
        ob_start();
...
$response->addHTML(
    PMA_Util::getListNavigator(
        $total_num_tables, $pos, $_url_params, 'mysql_structure.php',
...
// tables form
$response->addHTML(
    '<form method="post" action="mysql_structure.php" '
...
$response->addHTML(
    PMA_TableHeader($mysql_is_information_schema, $server_slave_status)
);
...
foreach ($tables as $keyname => $current_table) {
    ...
    list($current_table, $formatted_size, $unit, $formatted_overhead,
        $overhead_unit, $overhead_size, $table_is_view, $sum_size)
            = PMA_getStuffForEngineTypeTable(
                $current_table, $mysql_is_information_schema,
    ...
    list($browse_table, $search_table, $browse_table_label, $empty_table,
        $tracking_icon) = PMA_getHtmlForActionLinks(
            ...
            $titles, $truename, $mysql_is_information_schema, $url_query
        );

    if (! $mysql_is_information_schema) {
    ...
    list($html_output, $odd_row) = PMA_getHtmlForStructureTableRow(
        ...
        $browse_table, $tbl_url_query, $search_table, $mysql_is_information_schema,
...
$response->addHTML(
    PMA_getHtmlBodyForTableSummary(
        $num_tables, $server_slave_status, $mysql_is_information_schema, $sum_entries,
        $mysql_collation, $is_show_stats, $sum_size, $overhead_size, $create_time_all,
...
$response->addHTML(
    PMA_getHtmlForCheckAllTables(
        ...
        $mysql_is_information_schema, $hidden_fields
    )
);

...
if (! isset($db_charsets)) {
    $db_charsets = array();
    $db_collations_flat = array();
}
...
function PMA_SQP_parse($sql)
{
    ...
    global $db_charsets, $db_collations_flat;
            ...
            if ($t_prev == 'alpha' && $d_prev == 'DEFAULT' && $d_cur == 'b' && $t_next == 'quote_single') {
              ...
              && isset($PMA_SQPdata_function_name[$d_cur_upper])) {
                ...
                if ($d_cur_upper == 'BINARY'
                    ...
                    && in_array($d_cur, $db_charsets)
                ) {
                    $t_suffix = '_charset';
                }
            } elseif (in_array($d_cur, $db_charsets)
              || in_array($d_cur, $db_collations_flat)
              || ($d_cur{0} == '_' && in_array(substr($d_cur, 1), $db_charsets))) {
                $t_suffix = '_charset';
            } else {
                // Do nothing
            }

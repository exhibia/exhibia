    ...
    function getVariablesStatus()
    {
        ...
        while ($row = PMA_DBI_fetch_assoc($res)) {
            if (isset($variables[$row['Variable_name']])) {
                $db_vars[$row['Variable_name']] = $variables[$row['Variable_name']];
            } elseif (! $like
                && strpos(strtolower($row['Variable_name']), strtolower($this->engine)) !== 0
            ) {
                continue;
            }
            $db_vars[$row['Variable_name']]['value'] = $row['Value'];

            if (empty($db_vars[$row['Variable_name']]['title'])) {
                $db_vars[$row['Variable_name']]['title'] = $row['Variable_name'];
            }

            if (! isset($db_vars[$row['Variable_name']]['type'])) {
                $db_vars[$row['Variable_name']]['type']

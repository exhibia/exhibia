...
/**
 * converts charset of a mysql message, usually coming from db_error(),
...
function PMA_DBI_get_databases_full($database = null, $force_stats = false,
    ...
    if (! $GLOBALS['cfg']['Server']['DisableIS']) {
        ...
        $db_error = PMA_DBI_getError($link);
        if (! count($databases) && $GLOBALS['errno']) {
            PMA_Util::mysqlDie($db_error, $sql);
        }
        ...
        foreach ($GLOBALS['pma']->databases as $database_name) {
            ...
            if ($force_stats) {
                include_once './libraries/db_charsets.lib.php';
 ...
 * </code>
 *
 * @param string|db_result $result     query or mysql result
 ...
 * @param bool   $test_for_db_schema Whether 'mysql' schema should
 *                                      be treated the same as IS and DD
 *
 * @return bool
 */
function PMA_is_system_schema($schema_name, $test_for_db_schema = false)
{
    return strtolower($schema_name) == 'information_schema'
            ...
            || ($test_for_db_schema && !PMA_DRIZZLE && $schema_name == 'mysql');
}

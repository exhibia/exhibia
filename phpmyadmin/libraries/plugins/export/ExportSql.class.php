...
class ExportSql extends ExportPlugin
{
    ...
    public function exportFooter()
    {
        ...
        if (! empty($GLOBALS['asfile'])
            && isset($db_charset_map[$charset_of_file])
            && ! PMA_DRIZZLE
        ) {
            $foot .=  $crlf
    ...
    public function exportHeader()
    {
        ...
        if (! empty($GLOBALS['asfile']) && ! PMA_DRIZZLE) {
            ...
            if (isset($GLOBALS['charset_of_file'])
                && isset($db_charset_map[$GLOBALS['charset_of_file']])
            ) {
                // we got a charset from the export dialog
                $set_names = $db_charset_map[$GLOBALS['charset_of_file']];
            } else {
                // by default we use the connection charset
                $set_names = $db_charset_map['utf-8'];
            }
            $head .=  $crlf

...
if (! defined('PMA_MYSQL_CLIENT_API')) {
    $client_api = explode('.', db_get_client_info());
    define(
...
function PMA_DBI_real_connect($server, $user, $password, $client_flags,
    ...
    if (empty($client_flags)) {
        if ($cfg['PersistentConnections'] || $persistent) {
            $link = @db_pconnect($server, $user, $password);
        } else {
            $link = @db_connect($server, $user, $password);
        }
    } else {
        if ($cfg['PersistentConnections'] || $persistent) {
            $link = @db_pconnect($server, $user, $password, $client_flags);
        } else {
            $link = @db_connect($server, $user, $password, false, $client_flags);
        }
    }
...
function PMA_DBI_connect(
    ...
    // always use CLIENT_LOCAL_FILES as defined in db_com.h
...
function PMA_DBI_select_db($dbname, $link = null)
{
    ...
    return db_select_db($dbname, $link);
}
...
function PMA_DBI_real_query($query, $link, $options)
{
    if ($options == ($options | PMA_DBI_QUERY_STORE)) {
        ...
        return db_unbuffered_query($query, $link);
    } else {
        return db_query($query, $link);
    }
}
...
{
    return db_fetch_array($result, MYSQL_BOTH);
}
...
{
    return db_fetch_array($result, MYSQL_ASSOC);
}
...
{
    return db_fetch_array($result, MYSQL_NUM);
}
...
{
    return db_data_seek($result, $offset);
}
...
function PMA_DBI_getError($link = null)
{
    ...
    if (null !== $link && false !== $link) {
        $error_number = db_errno($link);
        $error_message = db_error($link);
    } else {
...
function PMA_DBI_get_fields_meta($result)
{
    ...
    for ($i = 0; $i < $num_fields; $i++) {
        ...
        $field->flags = db_field_flags($result, $i);
        $field->orgtable = db_field_table($result, $i);
        $field->orgname = db_field_name($result, $i);
        $fields[] = $field;
    }
    return $fields;
}
...
{
    return db_field_flags($result, $i);
}

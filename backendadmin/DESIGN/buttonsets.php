...
<?php
 ...
 if(db_num_rows(db_query("select * from sitesetting where name = 'buttons'")) >= 1){
    ...
    $old_button = db_fetch_array(db_query("select value from sitesetting where name = 'buttons'"));
			...
			<select id="buttons" name="buttons" onchange="ajax_PAS('include/addons/design_suite/DESIGN/buttonsets.php?', 'buttonset=' + $('#buttons').val() , 'get','button-preview');">
...
?>
	    ...
	    <td id="button-preview">
	      ...
	      $buttons = directoryToArray("../img/buttons/$old_button[0]", "*", true);
...
shell_exec("rm ../../../../img/buttons/.htaccess -f");
$handle = fopen("../../../../img/buttons/.htaccess", "w+");

  <input type="hidden" name="margin" id="margin" value="" />
   <input type="hidden" name="padding" id="padding" value="" />
  
  <?php $pm = array('margin', 'padding');
 $units = array('px', 'pt', 'em', 'mm', 'cm', 'in', 'pc', '%', 'auto', 'inherit');
			    ...
			    foreach($pm as $m){
			    ?>
			    <td valign="top" height="100%" style="vertical-align:top;text-align:center;border:1px dotted blue;border-radius:15px;padding:5px 5px 5px 5px;margin-right:2px;" width="100px" align="center" colspan="1" id="margin_box" class="control_text_less">
			   <h2 style="font-size:13px;margin:0 0 0 0;float:left;padding-bottom:10px;font-size:13px;"><?php echo ucfirst($m); ?></h2>
			   <br />
				...
				function update_these_<?php echo $m;?>s(divId, bType){
					  str = $(divId).css('<?php echo $m;?>-top');
					    ...
					    if(!str.match(/undefined/)){
					      document.getElementById('<?php echo $m; ?>').value = str;
					      }else{
					      str = '';
					      }
			...
			?>
				...
				<div style="text-align:left;padding-left:10px;width:100px;" class="edit_css_interfaces universal">
				    <label style="font-size:10px;float:left;width:100px;"><?php echo ucfirst($m); ?>-<?php echo ucfirst($side); ?>
			...
			?>
					...
					function update_align(divId){
					  ...
					  <div style="width:100px;position:relative;"  class="edit_css_interfaces universal">
					      ...
					      <select name="text-align" id="text-align"  onkeyup="javascript: update_align('<?php echo $_REQUEST['id'];?>', this.value);"  title="text-align" class="other">

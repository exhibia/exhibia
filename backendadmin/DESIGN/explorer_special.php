<?php
if(!file_exists($BASE_DIR . "/include/addons/commentbox/$template/img/")){
@mkdir($BASE_DIR . "/include/addons/commentbox/$template/");
@mkdir($BASE_DIR . "/include/addons/commentbox/$template/img");
@shell_exec("ln -s " . $BASE_DIR . "/uploads/banner " . $BASE_DIR . "/include/addons/commentbox/$template/img");
...
<div id="upload-button-logo2"></div>
 ...
 function deleteBanner(id, yesno){
	  ...
	  $('#alert_delete').dialog();
	    ...
	    $.post('<?php echo $SITE_URL; ?>/backendadmin/DESIGN/search_dir_special.php?delete=' + id, function(){
...
function createUploader(divId){
 ...
 var uploader = new qq.FileUploader({
    ...
    action: '<?php echo str_replace("backendadmin", "", $_SITE_URL);?>include/addons/uploader/uploadify_special.php',
    // validation
    ...
    params: { 
	      ...
	      <?php if(file_exists("$BASE_DIR/include/addons/commentbox/$template/img/")){ ?>
	      'path': '<?php echo $BASE_DIR;?>/include/addons/commentbox/<?php echo $template;?>/img/'},
	      <?php }else{ ?>

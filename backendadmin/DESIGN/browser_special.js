function browser(params){
	...
	var search=function(){
		...
		with (a){
			Method="POST";
			URL=siteurl + "backendadmin/DESIGN/search_dir_special.php";
	...
	var showFiles=function(res){
		...
		for (i=0;i<res.contents.length;i++){
			...
			$('#file_' + i ).qtip({content:{ text: '<img src="' + siteurl + '/include/addons/commentbox/' + template + '/img/'+f.fName+'" style="width:' + f.fWidth / 2 + 'px;height:auto;" />'}, 

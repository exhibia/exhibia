<?php
$conditionals = array("issset", "is_array", "empty", "!isset", "!is_array", "!empty", "==", "!=", "<>", ">=", "<=", ">", "<");
if(!empty($BASE_DIR)){
...
if(!empty($_POST['menu_name'])){
    ...
    if($_POST['link_text'][$key] == '[[FORUMS]]'){
    
    
    
      if($_POST['delete_link'][$key] == 'yes'){
    ...
    if($_POST['link_text'][$key] == '[[REDEMPTION]]'){
    ...
    if($_POST['link_text'][$key] == '[[COMMUNITY]]'){
	...
	db_query("update sitesetting set value = 0,status=0 where name = 'community'");
	  ...
	  if(db_num_rows(db_query("select * from navigation where id = '$key' and menu_name = '$_POST[menu_name]'")) >= 1){
	      
	      
		  db_query("update navigation set sort='" . $_POST['sort'][$key] . "', link='" . addslashes($_POST['link'][$key]) . "', link_text='" . addslashes($_POST['link_text'][$key]) . "' where id= '$key'");
		  ...
		  foreach($_POST['conditional_type'][$id] as $keyc => $valuec){
		
		      if(db_num_rows(db_query("select * from nav_conditionals where id = '$keyc'")) >= 1){
		      
		      
			if($_POST['delete_conditional'][$id][$keyc] != 'yes' ){
			
			    if(!empty($_POST['conditional_operator'][$id][$keyc]) & !empty($_POST['conditional_operator'][$id][$keyc])){
			      db_query("update nav_conditionals set link_name= '" . addslashes($_POST['link_text'][$id]) . "', conditional_type = '" . addslashes($valuec) . "', conditional_operator='". $_POST['conditional_operator'][$id][$keyc] . "', conditional_val='". addslashes($_POST['conditional_val'][$id][$keyc]) . "', table_id=$key where id=$keyc");
			  ...
			  if(!empty($_POST['conditional_operator'][$id][$keyc]) & !empty($_POST['conditional_operator'][$id][$keyc])){
			    ...
			    db_query("insert into nav_conditionals values(null, '" . addslashes($_POST['menu_name']) . "', link_name='". addslashes($_POST['link_text'][$id]) . "', conditional_type='" . addslashes($_POST['conditional_type'][$id][$keyc]) . "', conditional_operator='". $_POST['conditional_operator'][$id][$keyc] . "', conditional_val='". addslashes($_POST['conditional_val'][$id][$keyc]) . "', 'navigation', '$key');");
		...
		$last = db_fetch_object(db_query("select * from navigation where menu_name = '$_POST[menu_name]' order by sort desc limit 1"));
		
		db_query("insert into navigation values(null, '$_POST[menu_name]', '" . addslashes($_POST['link'][$key]) . "', '" . addslashes($_POST['link_text'][$key]) . "', '', '', '" . $last->sort + 1 . "');");
		$id = db_insert_id();
...
?>
<h3>Please choose a page area</h3>
<ul style="list-style-type:none;">
<?php

    $sqlm = db_query("select distinct(menu) from page_areas where menu like '%column%' order by id asc");

	while($rowm = db_fetch_array($sqlm)){
	
	    ?><li onclick="javascript:$('.module_editor').css('display', 'none');$('#<?php echo $rowm['menu'];?>_editor').css('display', 'block');">
	    
		<?php echo ucwords(str_replace("_", " ", $rowm['menu']));?>
		<ul style="display:none;list-style-type:none;" id="<?php echo $rowm['menu'];?>_editor" class="module_editor">
		    <?php
		    $sqlm2 = db_query("select distinct name, menu_index from page_areas where menu = '$rowm[menu]' order by menu_index, id asc");
			while($rowm2 = db_fetch_array($sqlm2)){
			?>
			
			<li style="border:1px dotted blue;margin:10px;padding:10px;"><a href=""><?php echo ucwords(str_replace("_", " ", $rowm2['name']));?></a>
	...
	<?php
      ...
      ?>
	    ...
	    <li style="margin-top:20px;">Add Link
			<ol  style="list-style-type:none;margin-left:-40px;"> 
			      <li style="display:inline;margin:10px 20px 5px 0;">
				  <input type="text" name="link_text[<?php echo $rowm['id'];?>  + 1]" id="link_text[<?php echo $rowm['id'];?> + 1]" value="<?php echo $rowm['link_text'];?>" />
			      </li>
			      <li style="display:inline;margin:10px 20px 5px 0;">
				  <textarea style="height:40px;width:150px;margin-top:10px;" name="link[<?php echo $rowm['id'];?> + 1]" id="link[<?php echo $rowm['id'];?> + 1]"><?php echo $rowm['link'];?></textarea>
			      </li>

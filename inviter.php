...
include 'data/inviterentry.php';

$inviterentry=InviterEntryFactory::getInviterEntry();

$openinviter_settings['username']=$inviterentry->getUsername();
$openinviter_settings['private_key']=$inviterentry->getApikey();
$openinviter_settings['message_body']=$inviterentry->getBody();
$openinviter_settings['message_subject']=$inviterentry->getSubject();
...
$contents=' <link rel="stylesheet" href="css/inviter.css" media="screen,projection" type="text/css" />';
...
$contents.="<form action='inviterwapper.php' method='POST' class='inviterform' name='openinviter'>" . ers($ers) . oks($oks);
if (!$done) {
    if ($step == 'get_contacts') {
        $contents.="<table align='center' id='formtable' class='thTable' cellspacing='5' cellpadding='5' style='border:none;'>
			<tr class='thTableRow'><td align='left'><label for='email_box'>Email:</label></td><td><input class='thTextbox' type='text' name='email_box' value='{$_POST['email_box']}'></td></tr>
			<tr class='thTableRow'><td align='left'><label for='password_box'>Password:</label></td><td><input class='thTextbox' type='password' name='password_box' value='{$_POST['password_box']}'></td></tr>
			<tr class='thTableRow'><td align='left'><label for='provider_box'>Email provider:</label></td><td><select class='thSelect' name='provider_box'><option value=''></option>";
        ...
        $contents.="</select></td></tr>
			<tr class='thTableImportantRow'><td colspan='2' align='right' style='height:40px;'><input style='margin:10px auto;' class='thButton' type='submit' name='import' value='Import Contacts'></td></tr>
        ...
        $contents.="<table class='thTable' cellspacing='0' cellpadding='0' style='border:none;'>
				<tr class='thTableRow'><td align='right' valign='top'><label for='message_box'>Your Message</label></td><td><textarea rows='5' cols='50' name='message_box' class='thTextArea' style='width:300px;'>{$_POST['message_box']}</textarea></td></tr>
...
if (!$done) {
    if ($step == 'send_invites') {
        if ($inviter->showContacts()) {
            $contents.="<table class='thTable' align='center' width='100%' cellspacing='5' cellpadding='3'><tr class='thTableHeader'><td colspan='" . ($plugType == 'email' ? "3" : "2") . "'>Your contacts</td></tr>";
            if (count($contacts) == 0)
                ...
                $contents.="<tr class='thTableFooter'><td colspan='" . ($plugType == 'email' ? "3" : "2") . "' style='padding:3px;'><input style='margin:10px auto;' type='submit' name='send' value='Send invites' class='thButton'></td></tr>";
            }
            $contents.="</table>";
        }

...
if($_REQUEST['params']['username']!="guest" && $userid < 10000000) {
	...
	$sql = ("select recentlist,highscorelist from cometchat_games where userid = '".mysqli_real_escape_string($GLOBALS['dbh'],$userid)."'");
	...
	$sql = ("insert into cometchat_games (userid,score,games,recentlist,highscorelist) values ('".mysqli_real_escape_string($GLOBALS['dbh'],$userid)."','".mysqli_real_escape_string($GLOBALS['dbh'],$gamescore)."','1','".mysqli_real_escape_string($GLOBALS['dbh'],$recentlist)."','".mysqli_real_escape_string($GLOBALS['dbh'],$highscorelist)."') on duplicate key update games = games+1 , score= score+'".mysqli_real_escape_string($GLOBALS['dbh'],$gamescore)."', recentlist = '".mysqli_real_escape_string($GLOBALS['dbh'],$recentlist)."' ,highscorelist ='".mysqli_real_escape_string($GLOBALS['dbh'],$highscorelist)."'");

...
function loadCCPopup(url,name,properties,width,height,title,force,allowmaximize,allowresize,allowpopout) {
	...
	if (jqcc(document).fullScreen() !== null && allowmaximize ==1) {
		jqcc('#cometchat_container_'+name).find('.cometchat_iframe').addClass('cometchat_iframe_'+name);
			jqcc('#cometchat_container_'+name+' .cometchat_maxwindow').click(function() {
	...
	if (allowpopout == 1) {
		jqcc('#cometchat_container_'+name+' .cometchat_popwindow').click(function() {
...
*/
        ...
        $.cometchat = $.cometchat || function(){
            ...
            var baseUrl = '/include/addons/cometchat/';
            var ccvariable = [];
            var mobile = 0;
            var language = ["Chat Options","Type your status and hit the enter key!","My Status","Available","Busy","Invisible","","","Please login to use chat","Chat Now","Me","Offline","Who's Online","Disable sound notifications","No users online at the moment. Please try again later.","New Messages...","","Offline","Find a user","<br\/><span style='color:#999'>is online<\/span>","<br\/><span style='color:#999'>is offline<\/span>","<br\/><span style='color:#999'>is online (busy)<\/span>","Set my status","I am...","Disable popup notifications","","Start chatting!","Close this bar","No users online at the moment. Please try again later.","No friends online at the moment. Please try again later.","I'm available","I'm busy","I'm offline","I'm offline","I'm away"," | ","And "," more notifications","This popup is already open. Please close the existing popup and try again.","Navigating away from this page will close your chat popups.","Others","Loading...","","My Name","Set my name","Type your Name\/Status and hit the enter key!","Messages from","Click here to view","New Message from","Updated","Failed","Chat Now","You have received a new chat message. Click OK to read","Please don't spam your friend","New messages","Send a message...","Sorry you have logged out","No change"];var trayicon = {"chatrooms":["chatrooms","Chatrooms","\/include\/addons\/cometchat\/modules\/chatrooms\/index.php","_lightbox","560","300","","1","1"],"announcements":["announcements","Announcements","\/include\/addons\/cometchat\/modules\/announcements\/index.php","_lightbox","280","300","","1",""],"scrolltotop":["scrolltotop","Scroll To Top","javascript:jqcc.cometchat.scrollToTop();","","","","","",""]};var settings = {"barWidth":960,"barAlign":"center","autoLoadModules":1,"barType":"fluid","plugins":["smilies","clearconversation","chathistory","report"],"extensions":["mobileapp","jabber","mobilewebapp"],"hideOffline":1,"autoPopupChatbox":1,"messageBeep":1,"beepOnAllMessages":1,"barPadding":20,"minHeartbeat":3000,"maxHeartbeat":12000,"fullName":0,"searchDisplayNumber":10,"thumbnailDisplayNumber":40,"typingTimeout":10000,"idleTimeout":300,"displayOfflineNotification":1,"displayOnlineNotification":1,"displayBusyNotification":1,"notificationTime":5000,"announcementTime":15000,"scrollTime":1,"armyTime":0,"disableForIE6":0,"iPhoneView":0,"hideBarCheck":1,"startOffline":0,"fixFlash":0,"lightboxWindows":1,"sleekScroller":1,"color":"exhibia","cookiePrefix":"cc_","disableForMobileDevices":1,"googleDesktopNotifications":1,"windowTitleNotify":1,"floodControl":100,"windowFavicon":0,"theme":"standard","mobiletab":1};  
...
(function($){  
	$.ccmobiletab = (function() {
		var title = "Chat Now", timestamp = 0, amount = 0, loggedout=0, cookie_prefix='cc_', mobileDevice = navigator.userAgent.match(/ipad|ipod|iphone|android|windows ce|blackberry|palm|symbian/i); var setalert = '1'
		...
		if($('#chat_stream').length >0 ){
            ...
            statusSendMessage: function(statustextarea) {
                ...
                if (message != '' && oldMessage != message) {
                    ...
                    $('.cometchat_statusbutton').text('No change');
                    setTimeout(function(){
                        $('.cometchat_statusbutton').text('Set my status');
                    }, 1500);   
                }
            },
            statusSendMessageSuccess: function(statustextarea){
                $(statustextarea).blur();
                setTimeout(function(){
                    $('.cometchat_statusbutton').text('Updated');
                }, 1800);
                setTimeout(function(){
                    $('.cometchat_statusbutton').text('Set my status');
                }, 2500);                
            ...
            statusSendMessageError: function(){
                setTimeout(function(){
                    $('.cometchat_statusbutton').text('Failed');
                }, 1800);
                setTimeout(function(){
                    $('.cometchat_statusbutton').text('Set my status');
                }, 2500);                
            ...
            setGuestName: function(guestnametextarea) {
                ...
                if (guestname != '' && oldguestname != guestname) {
                    ...
                    $('.cometchat_guestnamebutton').text('No change');
                    setTimeout(function(){
                        $('.cometchat_guestnamebutton').text('Set my name');
                    }, 1500);
                }
            },
            setGuestNameSuccess: function(guestnametextarea) {
                ...
                setTimeout(function(){
                    $('.cometchat_guestnamebutton').text('Updated');
                }, 1800);
                setTimeout(function(){
                    $('.cometchat_guestnamebutton').text('Set my name');
                }, 2500);                
            ...
            setGuestNameError: function() {
                setTimeout(function(){
                    $('.cometchat_guestnamebutton').text('Failed');
                }, 1800);
                setTimeout(function(){
                    $('.cometchat_guestnamebutton').text('Set my name');
                }, 2500);                
        ...
        return {
			...
			init: function (id, mode) {
				...
				if (chatroommode != 0) {
					baseUrl = $.cometchat.getBaseUrl();
					basedata = $.cometchat.getBaseData();			
					$[$.cometchat.getChatroomVars('calleeAPI')].loadCCPopup(baseUrl+'plugins/chathistory/index.php?embed=web&chatroommode=1&logs=1&history='+id+'&basedata='+basedata, 'chathistory',"status=0,toolbar=0,menubar=0,directories=0,resizable=0,location=0,status=0,scrollbars=0,width=650,height=480",650,480,'Chat History');				
				} else {
					baseUrl = $.cometchat.getBaseUrl();
					baseData = $.cometchat.getBaseData();
					loadCCPopup(baseUrl+'plugins/chathistory/index.php?embed=web&history='+id+'&logs=1&basedata='+baseData, 'chathistory',"status=0,toolbar=0,menubar=0,directories=0,resizable=0,location=0,status=0,scrollbars=1, width=650,height=480",650,480,'Chat History'); 
				}
			}
			...
			init: function (id, mode) {
				...
				if ($("#cometchat_user_"+id+"_popup .cometchat_tabcontenttext").html() != '') {
					...
					if (mode === 'mobilewebapp') {
						...
						loadCCPopup(baseUrl+'plugins/report/index.php?id='+id+'&basedata='+baseData, 'report',"status=0,toolbar=0,menubar=0,directories=0,resizable=0,location=0,status=0,scrollbars=0, width=430,height=220",430,175,'Reason as to why you are reporting this user?'); 
					}	
				} else {
					alert('Sorry, your conversation with this user is empty.');
				}
...
(function($) {
    $.ccjabber = (function() {
        var session = '';
        var logout = {};
        logout['facebook'] = 'Logout from Facebook';
        logout['gtalk'] = 'Logout from Gtalk';
        var ccjabber = [];
        ...
        ccjabber.login = 'Connect to Facebook/ Gtalk<div style="clear:both"></div>';
        ccjabber.theme = 'standard';
        ccjabber.crossDomain = '0';
        ccjabber.server = '//j.chatforyoursite.com/j';
        ...
        return {
            ...
            login: function() {
                ...
                loadCCPopup(baseUrl + 'extensions/jabber/index.php?basedata=' + baseData + '&basedomain=' + baseDomain, 'jabber', "status=0,toolbar=0,menubar=0,directories=0,resizable=0,location=0,status=0,scrollbars=0, width=410,height=100", 410, 100, 'Connect to Facebook/ Gtalk  Chat');
            },
            logout: function() {
            ...
            sendMessage: function(id, message) {
                ...
                $[ccjabber.theme].addMessage(id, message, 1, 0, currenttime, 1, null);
                            ...
                            if (data) {
                                if (data[0] && data[0].error == '1') {
                                    ...
                                    $.each(data, function(id, message) {
                                        ...
                                        $[ccjabber.theme].addMessage(message.from, message.msg, 0, 0, message.time, 0, null);
                                        ccjabber.heartbeatTime = ccjabber.minHeartbeat;
                                    });

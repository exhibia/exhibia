...
jqcc.extend($.mobilewebapp, {
	...
	displayChatroomMessage: function (item, fetchedUsers) {	
		...
		if (temp != '') {
			if(jqcc.cometchat.getChatroomVars('prepend') == 0){
				...
				setTimeout(function () {jqcc.mobilewebapp.crscrollToBottom();},500);
			}else{
	...
	ChatroomMessage: function (id,incomingmessage,self,old,incomingid,selfadded,sent){
		...
		if(selfadded == 0){  
			...
			if ($("#cometchat_message_"+self).length > 0) {
				$("#cometchat_message_"+self+' .cometchat_chatboxmessagecontent').html(incomingmessage);
			} else {
				sentdata = '';				
				if (incomingid != null) { 
	...
	loadMobileLobbyReverse:function (){
		jqcc.mobilewebapp.loadLobbyReverse();exit;
	},
